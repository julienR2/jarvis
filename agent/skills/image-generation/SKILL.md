...
description: Generate AI images using the ImageRouter API. Use when the user asks to generate, create, draw, or make an image, picture, illustration, or artwork.
...
```
YOUR_API_KEY=${IMAGEROUTER_API_KEY}
```

Always set this variable at the top of any curl command.
...
After generating an image, download it to `/jarvis/workspace/uploads/` and reference it in your response:

```bash
curl -o /jarvis/workspace/uploads/generated-image.webp "IMAGE_URL_FROM_RESPONSE"
```

Then show it: `![description](/jarvis/workspace/uploads/generated-image.webp)`
